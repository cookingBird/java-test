...
public class ReflectGetPackageClasses {

    public static void main(String[] args) throws Exception {
        Set<Class<?>> classes = getAllClasses("com.example.java8test.reflect", Optional.empty());
        classes.stream().map(Class::getName).forEach(System.out::println);
    ...
    public static Set<Class<?>> getAllClasses(String pkgName, Optional<Boolean> recursive) throws IOException {
        // class类的集合
        Set<Class<?>> classes = new LinkedHashSet<Class<?>>();
        // 是否循环迭代
        boolean recur = recursive.orElse(true);
        // 获取包路径
        ...
        while (dirs.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                ...
                findClassesInPackageByFile(pkgName, filePath, recur, classes);
            } else if ("jar".equals(protocol)) {
                // 获取jar
                JarFile jar = ((JarURLConnection) url.openConnection()).getJarFile();
                // 从此jar包 得到一个枚举类
                Enumeration<JarEntry> entries = jar.entries();
                findClassesInPackageByJar(pkgName, entries, pkgDirName, recur, classes);
            }
        }
